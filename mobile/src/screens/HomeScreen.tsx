...
export default function HomeScreen({ navigation }: any) {
  ...
  const toggleFavorite = async (noteId: string) => {
    if (!isLoggedIn)
      return Alert.alert("Error", "Must be logged in to favorite notes");
    try {
  ...
  const reportNote = async (noteId: string) => {
    ...
    try {
      ...
      Alert.alert(
        "Reported",
        "Thank you for your report. Admins will review it.",
      );
    } catch (e: any) {
  ...
  const renderItem = ({ item }: { item: any }) => (
    <TouchableOpacity
      ...
      activeOpacity={0.8}
    >
      <View style={{ position: "absolute", top: 12, right: 12, zIndex: 10, flexDirection: 'row', alignItems: 'center' }}>
        <TouchableOpacity
      ...
      <View style={styles.cardHeader}>
        ...
        <Text
          style={[styles.schoolText, { color: colors.text }]}
          numberOfLines={1}
        >
          {item.schoolName}
        </Text>
      ...
      <View style={styles.cardFooter}>
        ...
        >
          <Ionicons
            ...
            size={22}
            color={
              favorites.includes(item.id) ? "#ef4444" : colors.textSecondary
            }
          />
        </TouchableOpacity>
        <View style={{ flexDirection: "row", alignItems: "center" }}>
              ...
              >
                <Text
                  style={[
                    ...
                    {
                      color:
                        selectedSchool === item ? "#fff" : colors.textSecondary,
                    },
                  ]}
                >
                  {item}
